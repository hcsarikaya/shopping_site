...
const UserDetail = ({ typeOfUser, user }: { typeOfUser: string, user: UserDetailType }) => {
    ...
    return (
        <div className='pb-12 lg:px-12 lg:pt-0 pt-4 flex lg:flex-row flex-col items-start gap-2 lg:gap-9 max-w-[660px] mx-auto lg:max-w-full'>
            ...
            <div className='relative flex-1 flex flex-col gap-4 w-full'>
                ...
                />}
                {activeTab === 'activity' && <UserActivity username={userDetail.username}/>}
                {activeTab === 'bookmarks' && <UserBookmarks username={userDetail.username}/>}
                {activeTab === 'blogs' && <UserBlogs username={userDetail.username}/>}
                {activeTab === 'products' && <UserProducts username={userDetail.username}/>}
            </div>
        </div>
    )
}
